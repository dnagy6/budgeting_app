...
from source.models import Transaction, Category, SavingsAccount, Budget
...
class TestCategoryLogic(unittest.TestCase):
    ...
    def test_actual_amount_sums_non_transfers(self):
        ...
        category = Category(name="Groceries", planned_amount=300.0)
        
        t1 = Transaction(amount=50.0, is_transfer=False)
        t2 = Transaction(amount=25.0, is_transfer=False)
        t_transfer = Transaction(amount=100.0, is_transfer=True)  # Should be ignored
        
        category.transactions.extend([t1, t2, t_transfer])
    ...
    def test_remaining_amount_calculation(self):
        ...
        category = Category(name="Utilities", planned_amount=150.0)
        category.transactions.append(Transaction(amount=40.0))
